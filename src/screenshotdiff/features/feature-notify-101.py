"""Notify handling for ScreenshotDiff."""
...
class NotifyHandler:
 """Processes notify requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty notify payload")
 return {"notify": str(payload)[:200], "attempts": 0}
...
def run_notify(payload, **options):
 """Module-level convenience entry point."""
 return NotifyHandler().run(payload, **options)